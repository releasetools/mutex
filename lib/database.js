...
import { describePosture, explainSslFailure, isDirectNegotiationFailure, resolveConnection, } from "./connection.js";
...
export class DatabaseMutex {
    ...
    async listLocks(owner = null) {
        return this.withSchemaRetry("Listing locks", async () => {
            const result = await this.pool.query(format(`SELECT ${LOCK_COLUMNS} FROM %I${owner === null ? "" : " WHERE owner = $1"} ORDER BY id;`, TABLE_NAME), owner === null ? [] : [owner]);
    ...
    async acquireLockInternal(name, reason, owner, expiration) {
        ...
        // the final result.
        const query = format(`WITH gate AS (
