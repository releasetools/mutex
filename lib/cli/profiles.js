...
export function profilesDirectory(env = process.env, home = os.homedir()) {
    return env.XDG_CONFIG_HOME
        ? path.join(path.resolve(env.XDG_CONFIG_HOME), "releasetools-mutex")
        : path.join(home, ".config", "releasetools-mutex");
}
export function profilesPath(env = process.env, home = os.homedir()) {
...
export function parseProfiles(text, filePath = PROFILES_FILENAME, requireDefault = true) {
    ...
    const profiles = [...sections].map(([name, values]) => validateProfile(name, values, filePath));
    const defaults = profiles.filter((profile) => profile.isDefault);
    if (requireDefault && defaults.length !== 1) {
        throw new ConfigurationError(`${filePath} must define exactly one default profile; found ${defaults.length}`, "Run 'mutex profile NAME' to choose one.");
    }
    return profiles;
}
export function formatProfiles(profiles) {
    return `${profiles
        ...
        if (profile.sslNegotiation) {
            lines.push(`ssl_negotiation = ${JSON.stringify(profile.sslNegotiation)}`);
        }
        return lines.join("\n");
    })
        .join("\n\n")}\n`;
}
export async function loadProfiles(filePath = profilesPath(), requireDefault = true) {
    try {
...
export async function ensureProfiles(input = process.stdin, output = process.stderr, filePath = profilesPath()) {
    ...
    if (input.isTTY) {
        ...
        try {
            const answer = await prompt.question(`Working directory [${suggested}]: `);
            if (answer.trim()) {
...
export async function selectProfile(requestedName, filePath = profilesPath()) {
    const loaded = await loadProfiles(filePath);
    if (!loaded) {
        if (requestedName) {
            throw new ConfigurationError(`profile '${requestedName}' is not defined because ${filePath} does not exist`, "Run 'mutex profile' to create it.");
        }
        ...
        throw new ConfigurationError(`no mutex profile and ${CONNECTION_ENV_VAR} is not set`, "Run 'mutex profile' to configure the server, or export MUTEX_DATABASE_URL for direct access.");
    }
    ...
    if (!profile) {
        throw new ConfigurationError(`profile '${requestedName}' is not defined`, `Defined profiles: ${loaded.profiles.map(({ name }) => name).join(", ")}`);
    }
    return { profile, configPath: loaded.path };
}
export async function setDefaultProfile(name, filePath = profilesPath()) {
    const loaded = await loadProfiles(filePath, false);
    if (!loaded) {
        throw new ConfigurationError(`${filePath} does not exist`, "Run 'mutex profile' to create it.");
    }
...
export function formatProfileList(profiles) {
    return `${profiles
        .map((profile) => `${profile.isDefault ? "*" : " "} ${profile.name} (${profile.mode})`)
        .join("\n")}\n`;
}
export async function chooseProfile(loaded, input = process.stdin, output = process.stderr) {
    if (!input.isTTY || typeof input.setRawMode !== "function") {
        output.write(formatProfileList(loaded.profiles));
        return null;
    }
    let selected = Math.max(loaded.profiles.findIndex((profile) => profile.isDefault), 0);
    ...
    const render = (first = false) => {
        ...
        for (const [index, profile] of loaded.profiles.entries()) {
            output.write(`\x1b[2K${index === selected ? ">" : " "} ${profile.name} (${profile.mode})\n`);
        }
    };
    output.write("Select a mutex profile with ↑/↓ and Enter:\n");
    render(true);
    return new Promise((resolve, reject) => {
        const finish = (name, error) => {
...
export async function profileCommand(name) {
    if (name) {
    ...
    if (selected) {
        ...
        process.stderr.write(`Default profile set to '${selected}' in ${loaded.path}\n`);
    }
}
...
function validateProfile(name, values, filePath) {
    ...
    for (const key of Object.keys(values)) {
        if (!allowed.has(key)) {
            throw new ConfigurationError(`unknown setting '${key}' in [${name}] of ${filePath}`);
        }
    }
    if (values.ssl_negotiation !== undefined &&
        values.ssl_negotiation !== "postgres" &&
        values.ssl_negotiation !== "direct") {
        throw new ConfigurationError(`[${name}] in ${filePath} needs ssl_negotiation = "postgres" or "direct"`, "direct saves a round trip and requires PostgreSQL 17 or newer.");
    }
    const sslNegotiation = values.ssl_negotiation;
    if (values.mode !== "server" && values.mode !== "direct") {
        throw new ConfigurationError(`[${name}] in ${filePath} needs mode = "server" or "direct"`);
    }
    if (typeof values.default !== "boolean") {
        throw new ConfigurationError(`[${name}] in ${filePath} needs default = true or false`);
    }
    if (values.mode === "direct") {
        if (values.bind_address !== undefined || values.working_dir !== undefined) {
            throw new ConfigurationError(`[${name}] is direct and must not define bind_address or working_dir`);
        }
        return {
            name,
            mode: "direct",
    ...
    if (!path.isAbsolute(values.working_dir)) {
        throw new ConfigurationError(`[${name}] working_dir must be an absolute path`);
    }
    return {
        name,
        mode: "server",
...
function parseValue(raw, filePath, index) {
    if (raw === "true")
        return true;
    if (raw === "false")
        return false;
    ...
    throw profileError(filePath, index, "values must be quoted strings or lowercase booleans");
}
...
function profileError(filePath, zeroBasedLine, message) {
...
export async function profileFileExists(filePath = profilesPath()) {
    try {
