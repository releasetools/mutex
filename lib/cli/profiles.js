...
export function parseProfiles(text, filePath = PROFILES_FILENAME, requireEnabled = true) {
    ...
    if (requireEnabled && enabled.length !== 1) {
        throw new ConfigurationError(`${filePath} must enable exactly one profile; found ${enabled.length}`, "Run 'mutex profile NAME' to choose one.");
    }
    return profiles;
}
...
export async function loadProfiles(filePath = profilesPath(), requireEnabled = true) {
    try {
...
export async function activateProfile(name, filePath = profilesPath()) {
...
export function formatProfileList(profiles) {
    return `${profiles
        .map((profile) => `${profile.enabled ? "*" : " "} ${profile.name} (${profile.mode})`)
        .join("\n")}\n`;
}
export async function chooseProfile(loaded, input = process.stdin, output = process.stderr) {
    ...
    let selected = Math.max(loaded.profiles.findIndex((profile) => profile.enabled), 0);
...
function validateProfile(name, values, filePath) {
    ...
    if (typeof values.enabled !== "boolean") {
        throw new ConfigurationError(`[${name}] in ${filePath} needs enabled = true or false`);
    }
