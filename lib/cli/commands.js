...
import { EXIT_ERROR, EXIT_NO_PROGRAM, EXIT_NOT_EXECUTABLE, EXIT_OK, EXIT_REFUSED, EXIT_UNAVAILABLE, } from "./exit-codes.js";
import { describeLockAction, describeOwnerMismatch, describeRecord, summarizeRecord, } from "./output.js";
 ...
 */
export async function commandLock(ctx, identifier, program, command = "lock") {
    const result = await tryLock(requestFor(ctx, identifier, command), ctx.mutex, ctx.log, {
...
/** `mutex unlock`. */
export async function commandUnlock(ctx, identifier) {
 ...
 */
export async function commandRenew(ctx, identifier) {
...
export async function commandStatus(ctx, identifier) {
 ...
 */
export async function commandList(ctx) {
    const owner = ctx.options.owner;
    const records = await ctx.mutex.listLocks(owner);
    ctx.out.result({ command: "list", owner, count: records.length, locks: records }, records.length === 0
        ? owner
...
/** `mutex prune`. */
export async function commandPrune(ctx) {
...
function requestFor(ctx, identifier, operation) {
    return {
        identifier,
