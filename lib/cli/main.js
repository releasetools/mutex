...
import { commandList, commandLock, commandPrune, commandRenew, commandStatus, commandUnlock, } from "./commands.js";
import { resolveConnectionString } from "./config.js";
import { ConfigurationError, EXIT_CONFIGURATION, EXIT_ERROR, EXIT_OK, EXIT_USAGE, UsageError, } from "./exit-codes.js";
import { Output } from "./output.js";
import { readPackageVersion } from "../version.js";
import { profileCommand, selectProfile } from "./profiles.js";
import { TcpMutexStore } from "../server/tcp-store.js";
import { serverCommand } from "../server/lifecycle.js";
export async function main(argv) {
    ...
    if (commandLine.command === "profile" || commandLine.command === "server") {
        try {
            ...
            return await serverCommand(identifier, options.profile, options.json, log);
        }
        catch (error) {
    ...
    try {
        ...
        if (selected.profile.mode === "direct") {
            ...
            log.debug(`Using direct profile '${selected.profile.name}' and the connection string from $${CONNECTION_ENV_VAR}.`);
            ...
            log.debug(`Using server profile '${selected.profile.name}' at ${selected.profile.bindAddress}.`);
            mutex = new TcpMutexStore(selected.profile.bindAddress, undefined, undefined, selected.profile.name);
        }
...
function reportError(error, log) {
    if (error instanceof UsageError) {
        process.stderr.write(`mutex: ${error.message}\n\nRun 'mutex help' for usage.\n`);
        return EXIT_USAGE;
    }
