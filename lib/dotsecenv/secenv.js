...
export const SECRET_NAME_PATTERN = /^[A-Za-z_][A-Za-z0-9_]*(::[A-Za-z_][A-Za-z0-9_]*)?$/;
const SECRET_REFERENCE = /^\{dotsecenv\/(.*)\}$/;
export function parseSecenv(content, file) {
    const entries = [];
    const issues = [];
    content.split("\n").forEach((raw, index) => {
        const line = index + 1;
...
export async function readSecenv(file) {
 ...
 */
export function findSecenvFile(cwd = process.cwd()) {
