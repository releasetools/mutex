...
import { ensureProfiles, loadProfiles, profilesPath, selectProfile, } from "../cli/profiles.js";
...
export async function serverCommand(action, requestedProfile, json, log) {
    ...
    if (action === "start" || action === "run") {
        ...
        throw new ConfigurationError(`no mutex server configuration found at ${filePath}`, "Run 'mutex profile' to create it.");
    }
...
async function startServer(profile, log) {
    // Check the secret before detaching so a configuration error is immediate.
    resolveConnectionString();
    const client = new TcpMutexStore(profile.bindAddress, 2_000, undefined, profile.name);
    try {
        const status = await client.health();
        log.info(`mutex server '${profile.name}' is already running as PID ${status.pid} on ${status.bindAddress}`);
        return EXIT_OK;
    }
    catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        if (!message.startsWith("cannot reach mutex server") &&
            !message.includes("timed out")) {
            throw new Error(`something is already listening on ${profile.bindAddress}, but it did not pass the '${profile.name}' server health check: ${message}`);
        }
    ...
    try {
        ...
        throw new ConfigurationError(`working directory does not exist: ${profile.workingDir}`);
    }
    const child = spawn(process.execPath, [launcher, "server", "run", "-p", profile.name], {
    ...
    while (Date.now() < deadline) {
        if (spawnFailure.error) {
            throw new Error(`could not start mutex server: ${spawnFailure.error.message}`);
        }
        if (child.exitCode !== null || child.signalCode !== null) {
            throw new Error(`mutex server exited during startup (${child.signalCode ?? `status ${child.exitCode}`}); run 'mutex server run -p ${profile.name}' to see the error`);
        }
        try {
            const status = await client.health();
            log.info(`Started mutex server '${profile.name}' as PID ${status.pid} on ${status.bindAddress}.`);
    ...
    throw new Error(`mutex server did not become ready: ${lastError instanceof Error ? lastError.message : String(lastError)}\n` +
 ...
 */
export function describeServerVersion(reported, mine = readPackageVersion()) {
    const server = reported ?? "not reported";
    return server === mine ? server : `${server} (this mutex is ${mine})`;
}
async function showStatus(profile, json) {
    const status = await new TcpMutexStore(profile.bindAddress, undefined, undefined, profile.name).health();
    if (json) {
...
async function stopServer(profile, log) {
    const client = new TcpMutexStore(profile.bindAddress, undefined, undefined, profile.name);
    const status = await client.health();
    await client.stop();
    const deadline = Date.now() + 10_000;
    while (Date.now() < deadline) {
        try {
            await new TcpMutexStore(profile.bindAddress, 250, undefined, profile.name).health();
            await delay(100);
        }
        catch {
    ...
    throw new Error(`mutex server '${profile.name}' did not stop within 10 seconds`);
}
function assertServerProfile(profile) {
    if (profile.mode !== "server") {
        throw new ConfigurationError(`profile '${profile.name}' uses direct mode`, "Select a server profile with -p NAME.");
    }
    ...
    if (!profile.bindAddress || !profile.workingDir) {
        throw new ConfigurationError(`server profile '${profile.name}' needs bind_address and working_dir`);
    }
    const paths = serverPaths(profile);
    if (!paths.logPath || !paths.pidPath)
        throw new Error("invalid server paths");
}
