...
- The `enabled` profile setting is now named `default`; existing profiles files must rename it, and profiles with `default = false` remain selectable with `--profile`.
