...
- The mutex server's protocol version is now 2, so a server left running across the upgrade refuses commands by name until it is restarted with `mutex server stop && mutex server start`.
