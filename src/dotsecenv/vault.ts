...
interface RawHeader {
  ...
  secrets?: Record<string, { secret?: number; values?: number[] }>;
}
...
export function vaultPathFor(dir: string): string {
  return path.join(dir, VAULT_DIRECTORY, VAULT_FILENAME);
}

export function parseVault(content: string, vaultPath: string): Vault {
  ...
  if (marker !== HEADER_MARKER) {
    ...
    throw new DotsecenvError(
      `${vaultPath} is not a vault file (unexpected header marker)`,
      { kind: "parse" },
    );
  }
  ...
  try {
    header = JSON.parse(lines[1]) as RawHeader;
  } catch (cause) {
  ...
  if (lines[2].trim() !== DATA_MARKER) {
    throw new DotsecenvError(
      `${vaultPath} is not a vault file (missing data marker)`,
      { kind: "parse" },
    );
  }
...
export async function readVault(vaultPath: string): Promise<Vault | null> {
  let content: string;
  try {
...
function unsupportedVersion(
  ...
  return new DotsecenvError(
    `${vaultPath} uses vault format v${version ?? "?"}; only v${SUPPORTED_FORMAT_VERSION} is supported`,
    {
      kind: "vault",
...
function readRecord(lines: string[], lineNumber: number): RawRecord | null {
